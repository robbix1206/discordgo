...
var aliasStructTmpl = template.Must(template.New("aliasStruct").
	Parse(`// Code generated by \"eventhandlers\"; DO NOT EDIT
// See events.go
...
func main() {
	if len(os.Args) != 2 {
		...
		fmt.Println("Usage: eventhandlers source.go")
		os.Exit(1)
	}
	...
	fs := token.NewFileSet()
	parsedFile, err := parser.ParseFile(fs, os.Args[1], nil, 0)
	if err != nil {
	...
	buf.Reset()
	aliasStructTmpl.Execute(&buf, names)
	...
	err = ioutil.WriteFile(filepath.Join(dir, strings.ToLower("eventstructs.go")), src, 0644)
	if err != nil {
		log.Fatal(buf, "writing output: %s", err)
	}
}
