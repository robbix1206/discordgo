...
// Log is a helper function that wraps msglog for the Loggable interface
// This adds a check to insure the message is only logged
// if the log level is equal or higher than the
// message log level
func Log(l Loggable, msgL int, format string, a ...interface{}) {

	if msgL > l.GetLogLevel() {
		return
	}
